...
const API_URL = `${process.env.NEXT_PUBLIC_ADMIN_API}/api/homepage/notice`;
...
function NoticeCard({ notice }) {
  const { day, month, time } = formatNoticeDate(notice.createdAt);
...
function EmptyState({ type }) {
  return (
    <div className="flex min-h-[180px] items-center justify-center rounded-md bg-[#F7F7FA] px-4 text-center">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-400">
          There are currently no {type.toLowerCase()} items.
        </p>
      </div>
    </div>
  );
...
function TabbedList({ heading, tabs, noticesByType, maxHeight = "820px" }) {
  const [active, setActive] = useState(0);

  const activeTab = tabs[active];

  const rows = noticesByType[activeTab] || [];

  return (
    <section className="w-full bg-white px-4 py-12 sm:px-6 sm:py-16 md:px-8 md:py-20">
      <div className="mx-auto flex w-full max-w-[1450px] flex-col items-center gap-7 sm:gap-9 md:gap-12">
        {/* Heading */}
        <h2
          className="
            text-center
        ...
        <div className="w-full">
          ...
          >
            {tabs.map((tab, index) => {
              ...
              const count = noticesByType[tab]?.length || 0;
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${
                      isActive
                        ? "rounded bg-white text-[#018837] shadow-sm md:border-b-2 md:border-[#018837]"
                        : "text-black hover:bg-white/60 hover:text-[#018837]"
                    }
                  `}
                >
                  <span>{tab}</span>
  ...
  useEffect(() => {
    ...
    async function fetchNotices() {
      try {
        ...
        if (isMounted) {
          setError(
            err.message || "Something went wrong while loading notices.",
          );
        }
      } finally {
        if (isMounted) {
    ...
    return (
      <section className="w-full bg-white px-4 py-12 sm:px-6 md:px-8 md:py-20">
        <div className="mx-auto flex max-w-[1450px] flex-col items-center">
          <h2 className="text-center font-serif text-3xl font-bold text-[#018837] sm:text-4xl md:text-[40px]">
            Notice Board
          </h2>
          ...
          <div className="mt-10 grid w-full grid-cols-1 gap-3 md:grid-cols-2">
            {Array.from({ length: 6 }).map((_, index) => (
              <div
                key={index}
                className="flex animate-pulse gap-4 rounded-md bg-[#E8E8F4] p-5"
              >
                <div className="h-[70px] w-[75px] shrink-0 rounded bg-gray-300" />
      ...
      <section className="w-full bg-white px-4 py-12 sm:px-6 md:px-8 md:py-20">
        <div className="mx-auto max-w-[1450px]">
          <div className="rounded-lg border border-red-200 bg-red-50 p-6 text-center">
            <h2 className="font-serif text-3xl font-bold text-[#018837]">
              Notice Board
            </h2>
            ...
            <button
              ...
              className="mt-5 rounded bg-[#018837] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#016f2d]"
            >
              Try Again
            </button>
          </div>
