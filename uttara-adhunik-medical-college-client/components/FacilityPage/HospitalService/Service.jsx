...
const SectionHeading = ({ number, title }) => {
  return (
    <div className="flex items-center justify-center gap-3">
      <span className="font-['Bitter'] text-[70px] leading-[1] font-bold tracking-[0.01em] text-[#018837] sm:text-[85px] lg:text-[100px]">
        {number}
      </span>

      <h2 className="max-w-[220px] font-['Bitter'] text-2xl font-bold uppercase leading-[1.2] tracking-[0.01em] text-black sm:text-[28px] lg:text-[30px]">
        {title}
      </h2>
    </div>
  );
...
const ServiceCard = ({ service }) => {
  return (
    <article
      className={`
        flex min-h-[200px] flex-col justify-center items-start
        gap-2.5 rounded-[10px] p-[30px]
    ...
    >
      <div
        className={`
          flex h-[70px] w-[70px] shrink-0
          items-center justify-center rounded-full p-[14px]
      ...
      <div className="mt-0.5 flex w-full flex-col gap-[5px]">
        ...
        <p
          className={`
            font-['Inter'] text-[10px] font-normal leading-[12px]
            tracking-[0.01em]
    ...
    >
      {isLeft && (
        <img
          src={icon}
          alt=""
          className="hidden h-[100px] w-[100px] shrink-0 object-contain lg:block"
        />
      )}
      ...
      <h3
        className={`
          ...
          uppercase leading-[36px] tracking-[0.01em]
          text-black sm:text-[30px]
          ${!isLeft ? "text-right" : ""}
        `}
      >
        {title}
    ...
    <section className="w-full bg-white px-5 py-12 sm:px-8 sm:py-16 lg:px-20 lg:py-20">
      <div className="mx-auto flex w-full max-w-[1440px] flex-col items-center gap-12 sm:gap-14 lg:gap-[50px]">
        ...
        <div className="flex items-center justify-center px-2.5">
          <h1 className="text-center font-['Bitter'] text-[34px] font-bold leading-tight tracking-[0.01em] text-[#018837] sm:text-[42px] lg:text-[50px] lg:leading-[60px]">
        ...
        <section className="flex w-full flex-col items-center gap-8 sm:gap-10 lg:gap-[50px]">
          ...
          <div className="w-full bg-[rgba(1,136,55,0.1)] p-6 sm:p-8 lg:p-[50px]">
            <div className="flex flex-col gap-8 lg:flex-row lg:items-stretch lg:gap-[50px]">
              {/* Cards */}
              <div className="order-2 grid flex-1 grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3 lg:order-1">
                {surgicalRelated.map((service) => (
                  <ServiceCard key={service.title} service={service} />
                ))}
              </div>
