...
import { useState } from "react";
import FormList from "@/components/admission/form/PaperList";
import PaperForm from "@/components/admission/form/PaperForm";
import { useFormData } from "@/lib/DataFetch/Admission/Forms/SWRDataFetch";
import Modal from "@/components/ui/Modal";
import { toast } from "sonner";

export default function FormManager() {
  const { data: about, mutate, isLoading } = useFormData();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admission/form/${id}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      const url = editingAbout
        ? `${process.env.NEXT_PUBLIC_API_URL}/api/admission/form/${editingAbout._id}`
        : `${process.env.NEXT_PUBLIC_API_URL}/api/admission/form`;
