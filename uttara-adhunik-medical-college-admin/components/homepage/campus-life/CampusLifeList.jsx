...
export default function CampusLifeList({
  ...
  const handleImageError = (campusId) => {
    setImageErrors((prev) => ({ ...prev, [campusId]: true }));
  };
    ...
    return (
      <div className="text-center py-12 bg-gray-50 rounded-lg">
        <p className="text-gray-500 text-lg">
          No campuses section found. Add your first campus section.
        </p>
      </div>
    );
  }
      ...
      {campus.map((campus) => (
        ...
        >
          ...
          <div className="relative h-48 w-full bg-gray-100">
            {!imageErrors[campus._id] ? (
              <Image
                src={campus.image}
                alt={campus._id}
                fill
                className="object-cover"
                onError={() => handleImageError(campus._id)}
            ...
            {campus.image?.includes("cloudinary") && (
          ...
          <div className="p-4">
            {/* CTA Display */}
            {campus.cta && (campus.cta.text || campus.cta.href) && (
            ...
            <div className="flex items-center justify-between text-xs text-gray-400 mb-4">
              <span>ID: {campus._id.slice(-6)}</span>
              <span>
                Updated: {new Date(campus.updatedAt).toLocaleDateString()}
              </span>
            </div>
