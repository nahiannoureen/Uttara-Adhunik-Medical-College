...
export default function VisionList({ vision, onEdit, onDelete, isDeleting }) {
  const [imageErrors, setImageErrors] = useState({});

  const handleImageError = (visionId) => {
    setImageErrors((prev) => ({ ...prev, [visionId]: true }));
  };
    ...
    return (
      <div className="text-center py-12 bg-gray-50 rounded-lg">
        <p className="text-gray-500 text-lg">
          No vision found. Add your first vision section.
        </p>
      </div>
    );
  }
      ...
      {vision.map((vision) => (
        ...
        >
          ...
          <div className="relative h-48 w-full bg-gray-100">
            {!imageErrors[vision._id] ? (
              <Image
                src={vision.image}
                alt={vision.title}
                fill
                className="object-cover"
                onError={() => handleImageError(vision._id)}
            ...
            {vision.image?.includes("cloudinary") && (
          ...
          <div className="p-4">
            ...
            <div className="flex items-center justify-between text-xs text-gray-400 mb-4">
              <span>ID: {vision._id.slice(-6)}</span>
              <span>
