...
import VisionList from "@/components/about/vision-and-mission/VisionList";
import { useVisionAndMissionData } from "@/lib/DataFetch/About/VisionAndMission/SWRDataFetch";
import VisionForm from "@/components/about/vision-and-mission/VisionForm";
import Modal from "@/components/ui/Modal";
import { toast } from "sonner";

export default function VisionManager() {
  const { data: vision, mutate, isLoading } = useVisionAndMissionData();
  ...
  const handleDelete = async (id) => {
    if (
      !confirm(
        "Are you sure you want to delete this vision and mission section?",
      )
    )
      return;
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/about/vision-and-mission/${id}`,
        {
          method: "DELETE",
        },
      );
