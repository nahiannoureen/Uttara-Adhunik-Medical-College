...
export default function GalleryList({ gallery, onEdit, onDelete, isDeleting }) {
  const [imageErrors, setImageErrors] = useState({});

  const handleImageError = (galleryId) => {
    setImageErrors((prev) => ({ ...prev, [galleryId]: true }));
  };
      ...
      {gallery.map((gallery) => (
        ...
        >
          ...
          <div className="relative h-48 w-full bg-gray-100">
            {!imageErrors[gallery._id] ? (
              <Image
                src={gallery.image}
                alt={gallery.title}
                fill
                className="object-cover"
                onError={() => handleImageError(gallery._id)}
          ...
          <div className="p-4">
            ...
            <div className="flex items-center justify-between text-xs text-gray-400 mb-4">
              <span>ID: {gallery._id.slice(-6)}</span>
              <span>
