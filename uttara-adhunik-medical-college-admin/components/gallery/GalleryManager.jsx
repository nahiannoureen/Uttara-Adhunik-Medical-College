...
import { useGalleryData } from "@/lib/DataFetch/Gallery/SWRDataFetch";
import GalleryForm from "@/components/gallery/GalleryForm";
import Modal from "@/components/ui/Modal";
import { toast } from "sonner";

export default function GalleryManager() {
  const { data: gallery, mutate, isLoading } = useGalleryData();
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      const url = editingGallery
        ? `${process.env.NEXT_PUBLIC_API_URL}/api/gallery/${editingGallery._id}`
  ...
  return (
    <div className="">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Total Images:{" "}
            <span className="font-semibold">{gallery?.length || 0}</span>
          </p>
        </div>
      ...
      <Modal
        ...
        title={editingGallery ? "Edit Event" : "Add New Event"}
      >
        <GalleryForm
